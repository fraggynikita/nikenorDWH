...
def get_dates(**args) -> tuple[str, str]:
    ...
    start_date = args["data_interval_start"].strftime("%Y-%m-%d")
    end_date = args["data_interval_end"].strftime("%Y-%m-%d")

    return start_date, end_date

def from_api_to_s3(**args):
    ...
    logging.info(f"💻 Загрузка за: {start_date.strftime('%Y-%m-%d')}/{end_date.strftime('%Y-%m-%d')}")

    api_key = Variable.get("api_key")  
    url = f"https://newsapi.org/v2/everything?q=*&from={start_date.strftime('%Y-%m-%d')}&to={end_date.strftime('%Y-%m-%d')}&language=en&sortBy=popularity&PageSize=*&apiKey={api_key}"
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            save_path = f"s3://{MINIO_BUCKET}/{LAYER}/{SOURCE}/{start_date.strftime('%Y-%m-%d')}/{end_date.strftime('%Y-%m-%d')}_news_data.parquet"
